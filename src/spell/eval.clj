...
(def core-builtins
  ...
  {;; Math
   ...
   'subvec (fn
             ([v start] (subvec-preserving-first-line v start))
             ([v start end] (subvec-preserving-first-line v start end))),
   ...
   'future* (fn [thunk]
              (let [current-raw-var (resolve 'spell.runtime/*current-raw*)
                    ...
                    f (bound-fn []
                        (if current-raw-var
                          (with-bindings* (cond-> {current-raw-var nil} computation-var (assoc computation-var true))
