...
For coding tasks (bug fixes, feature implementation, test-driven work), consult (!describe reminders :coding) on the first turn for a research-plan-implement-verify workflow and specific guidance on dependencies, test verification, and common failure modes."
          :coding "CODING TASKS — Research, plan, implement, verify. Expect first attempts to fail; iterate until the real tests pass.
