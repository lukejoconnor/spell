...
(def math
  ...
  {:docs {;; Basic
          ...
          ;; Type predicates
          :NaN? "True if x is NaN"
          :infinite? "True if x is positive or negative infinity"
          ;; Auto-promoting arithmetic (for big numbers)
          :+' "Addition with auto-promotion to bigint"
          :-' "Subtraction with auto-promotion to bigint"
          :*' "Multiplication with auto-promotion to bigint"
          :inc' "Increment with auto-promotion"
          :dec' "Decrement with auto-promotion"
          ;; Type coercion
          :float "Coerce to float"
          :double "Coerce to double"
          :long "Coerce to long"
          :bigdec "Coerce to BigDecimal"
          :rationalize "Convert to exact rational"
          ;; Constants
          :PI "Pi (3.14159...)"
...
(check-result \"Capital of France?\" \"London\")  ; => {:wrong \"London is the capital of the UK, not France.\"}"}
   ;; check-result: verify answer with tools/leaf-llm, return {:ok answer} or {:wrong msg}
   :check-result {:spell/fn true
                  ...
                  :body '((let [verification-prompt (cat "Verify this answer.\n\n"
                                ...
                                response (tools/leaf-llm verification-prompt)
