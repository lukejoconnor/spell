...
floor, ceil, round, trunc, factorial, gcd, and lcm return integer values.
...
- After a failed attempt that created resources (files, API objects, containers), reuse or clean up existing resources instead of creating duplicates.
...
Example:
  (think \"Validation evidence: ran `python3 -m pytest tests/test_server.py::test_handles_empty_input -q` and it passed; output file `/app/out.json` now exists and contains the expected empty list.\")"
          :orchestrator "ORCHESTRATOR MODE - Use explicit orchestration, not just a basic single-agent loop.
...
Prefer !peek-now over !call-now for disposable tool calls (auto-appends prune that removes the binding on the following extension while leaving the !peek-now call visible):
...
On the subsequent turn, persist what you need before extending:
    ...
    '(!extend)
    ;; next turn: data is pruned, the !peek-now call remains visible, and targets survive as literals
...
When running a shell script or Python program that you do not need to rerun, keep it inside !peek-now:
    ...
    '(!extend)
    ;; next turn: the verify binding is gone; the !peek-now call remains visible
...
Use !peek when you want the binding to disappear on the following extension
unless you explicitly persist what you need. The !peek call itself remains
visible as prior work.

If the tool call is a disposable shell script or Python program, prefer !peek-now;
the binding(s) will be pruned on the next extension while the !peek-now call
stays visible. If you need to rerun the script later, write it to disk first
with io/write-file."
...
On your next extension, that prune removes the result binding(s) from source
while leaving the peek command visible. If you need part of the value, persist
it first with your own persist form.
...
Example:
  ...
  ;; next turn: code is pruned, the !peek-now call remains visible, and fn-defn remains"
