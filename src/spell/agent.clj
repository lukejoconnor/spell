...
;; Leaf LLM instance for tools namespace (created once at load time)
(def ^:private tools-leaf-llm
  (llm/make-leaf-llm {}))

(def tools-namespace
  "Tools namespace matching spell.core/tools."
  {:docs {:bash "Execute shell command. Returns {:exit N :out \"...\" :err \"...\"}."
   ...
   :leaf-llm tools-leaf-llm
   :make-form-llm llm/make-form-llm})
...
(def default-agent-def
  "Built-in default agent definition (equivalent to agents/default.agent.edn).
   ...
   Note: seqs, fns, and bit- ops are in core-builtins (matching Clojure)."
  {:name 'default
   :doc "Default agent with all standard tools"
   :namespaces {'tools 'stdlib/tools
                'strings 'stdlib/strings
                'math 'stdlib/math
                'patterns 'stdlib/patterns}
