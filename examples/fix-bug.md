...
```bash
spell -d 8 examples/fix-bug.spl
```
...
1. **Outer LLM** runs tests → sees `FAIL: average([1, 2, 3, 4, 5]) = 2.0, expected 3.0`
2. **Outer LLM** reads the file content
3. **Outer LLM** delegates to **child LLM** via `(llm (cat "Test failure: " test-output "\n\nSource:\n" file-source "\n\n..."))`
4. **Child LLM** analyzes the failure and returns a **data structure**: `{:old "..." :new "..."}`
5. **Outer LLM** extracts `:old` and `:new` from the result and calls `str-replace`
...
(str-replace path old new)
```

This pattern is more robust than having the child return executable code because:
- The child doesn't need to know about `str-replace` or how to call it
