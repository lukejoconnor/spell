...
The module joins a captured computation with `!ask-await`; the enclosing agent resumes with an actual `msg-N` from `:future`. Inspect that report rather than capturing a wait as if it were the next message. Keep evidence, actual edge/result receipts and source coordinates across compaction. Do not restart a still-running relay merely because an unrelated message arrived.
