...
Raw operational IO/web/MCP results use {:ok boolean :out payload :err text-or-nil}, plus :exit for processes and :status for HTTP when known. Serialization adds :truncated false, or :truncated true precisely when it omits returned content, without changing :ok/:exit/:status. An existing true flag survives reserialization. Third-party structured payloads stay under :out. Inspect failure/truncation before treating output as complete. Process stdout/stderr preserve whitespace; empty stderr is nil. grep exit 1 with empty stderr is valid no-match. Launch failures have :exit nil; existing timeouts retain :exit -1 and an explanatory error for incomplete execution, not truncation.
...
Character windows, like line ranges, select the request and do not set :truncated. Surrogate-splitting boundaries move inward. read-file preserves original terminators; read-lines strips them while retaining selected string rows. Each later file read is fresh evidence of CURRENT contents, not retrieval of the earlier value.
