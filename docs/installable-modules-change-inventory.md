...
| --- | --- | --- |
| Namespace short docs, guide, per-function help and invocation examples | [old host](https://github.com/lukejoconnor/spell/blob/cffd2933e7bae1f543a585020ceef3fa13554589/src/spell/patterns.clj) | [host source](../src/spell/patterns.clj), `def patterns`, `:short-docs`, `:docs`, `:guide` and the five verb docs (starts near line 162) |
...
| Namespace export/description wiring | [old stdlib](https://github.com/lukejoconnor/spell/blob/cffd2933e7bae1f543a585020ceef3fa13554589/src/spell/stdlib.clj) | [stdlib](../src/spell/stdlib.clj), `patterns` namespace wiring |
...
All old bundle bodies and embedded prompts are available as [complete git-before source](https://github.com/lukejoconnor/spell/blob/cffd2933e7bae1f543a585020ceef3fa13554589/config/spl-lib/patterns.spl). Each after link below is the complete editable module definition, including docs/requirements/source, prompt construction, and interpolation—not a truncated prompt excerpt.
...
No agent/model-profile capability changes are required. The live companion links the verified existing `io-tc.agent.edn` and `openai-tc.edn` configuration, pins model/reasoning/budget/depth/token limits, and explicitly leaves credential/provider/billing validation unclaimed.
...
The documentation worker prepared artifacts and used no paid subprocess, production/test edits, or commits. A fresh no-provider parse checked the complete live program. A separate fresh isolated TestProvider execution of the parsed artifact then passed (exit 0; three scripted responses), with actual root 11 → 12, peer 11 → 12 → 12, preserved edits/reuse receipts, complete entry keys, and body-free audit prefixes. These are deterministic/scripted checks, not real-model acceptance. Documentation source checks and `git diff --check` passed; the rendered documentation build is blocked by missing `vitepress` (exit 127). CLI help exited 0 and actual checked-out profile fields were inspected. Production/test owners provide their own integration receipts. Before pruning any review evidence, retain exact source slices, observed stored IDs/offsets, checkpoints, and actual effect receipts; never prune evidence then rediscover it.
