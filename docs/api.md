...
The API is small. A run supplies task input, chooses an agent, chooses an LM profile, and may override the small set of values users are likely to vary from run to run. LM profiles describe how Spell calls an LLM service. Agent configs describe the Spell runtime profile exposed to the model. In this document, provider means the API provider or backend implementation, such as OpenAI, Anthropic, Fireworks, Codex, Ollama, or the test provider.
...
(spell/run {:prompt "Answer with the number 42."
            :lm-profile "config/lm-profiles/openai-tc.edn"
...
| `:lm-profile` | yes | LM profile selection. Accepts an LM profile path, an inline LM profile map, or an already constructed low-level provider instance. |
| `:agent` | yes | Agent selection. Accepts a `.agent.edn` path. |

LM profile and agent are required at the API boundary. Config files may still name defaults, but those defaults are exposed as `:default-*` fields and resolved by higher-level helpers or CLIs rather than making `run` ambiguous.
...
|---|---:|---|
| `:model` | LM profile `:default-model` | Model override for this run. This is exposed because model selection is likely to vary during normal experimentation. |
| `:reasoning-effort` | LM profile `:default-reasoning-effort` | Reasoning-effort override for this run. This is exposed because users often sweep cost/quality tradeoffs. |
| `:budget` | agent `:default-budget` or runtime default | Maximum spend in dollars for the run. `nil` means use the configured default. `0` means unlimited. |
...
Use an inline LM profile map:

```clojure
(spell/run {:prompt "Return 42."
            :lm-profile {:provider :openai
                         :lm-profile-name "openai-tc"
...
LM profiles describe how to call an LLM service. They own endpoint, authentication, model-call policy, transport behavior, retry behavior, timeouts, pricing, and provider-specific request features.

Agent configs describe the Spell runtime profile exposed to the model. They own the system prompt, available Spell namespaces, sub-agent topology, output contract, and recovery behavior.

The test is:

- If changing the option changes the HTTP/API request, model transport, provider call controls, cost accounting, or retry policy, it belongs in the LM profile.
- If changing the option changes what Spell capabilities or instructions the model sees, or how Spell validates/evaluates the model's program, it belongs in agent config.
...
This means model-call policy lives in LM-profile defaults, while implementation leftovers are kept out of the public API/config surface.

## LM Profiles

LM profile files live under `config/lm-profiles/` and use EDN maps.
...
| `:lm-profile-name` | all | add LM profile | Human-readable LM profile name. Defaults to the profile filename without `.edn`. |
| `:api-key-env` | `:openai`, `:anthropic-pf`, `:anthropic-tc`, `:fireworks`, `:fireworks-tc` | keep LM profile | Environment variable that contains the API key. Public config files prefer this over literal secrets. |
| `:base-url` | `:openai`, `:codex-tc`, `:fireworks`, `:fireworks-tc`, `:ollama` | keep LM profile | API base URL. |
| `:model` | all model-backed providers | rename to `:default-model` | LM profile default model. `spell.api/run :model` may override it for a run. |
| `:default-reasoning-effort` | `:openai`, `:codex-tc`, `:anthropic-pf`, `:anthropic-tc`, `:fireworks`, `:fireworks-tc` | add LM profile | Provider-neutral reasoning setting. The provider maps it to its native request shape, including Anthropic thinking budgets. `spell.api/run :reasoning-effort` may override it for a run. |
| `:max-tokens` | all hosted model providers | keep LM profile | Maximum response tokens requested from the provider. |
| `:retries` | all hosted model providers | move to LM profile | Provider retry schedule for transient API failures. Values are sleep durations in seconds. |
| `:request-timeout-sec` | `:openai`, `:anthropic-pf`, `:anthropic-tc`, `:fireworks`, `:fireworks-tc` | keep LM profile | Per-request timeout in seconds. |
| `:sse-idle-timeout-sec` | `:anthropic-pf`, `:anthropic-tc`, `:fireworks`, `:fireworks-tc` | keep LM profile | Streaming timeout in seconds with no received bytes. |
| `:sse-completion-timeout-sec` | `:anthropic-pf`, `:anthropic-tc`, `:fireworks`, `:fireworks-tc` | keep LM profile | Total streaming response timeout in seconds. |
| `:costs` | all | keep LM profile | Pricing overrides merged into `data/pricing.edn`. |
| `:cache-read-ratio` | all | keep LM profile | Cost-table helper used when deriving cache-read prices from base input prices. |
| `:default-agent` | all | keep LM profile | Agent profile that higher-level helpers may use when no agent is supplied. `spell.api/run` still requires `:agent`. |
| `:use-responses-api` | `:openai` | keep LM profile | Force OpenAI Responses API instead of Chat Completions. |
| `:force-tool-call` | `:openai` | keep LM profile | Require a `spell_suffix` custom tool call. |
| `:prompt-cache-key` | `:openai`, `:codex-tc` | keep LM profile | Stable prompt cache key reused across model calls when cache-prefixing is enabled. |
| `:verbosity` | `:openai`, `:codex-tc` | keep LM profile | Provider-facing verbosity control. |
...
|---|---|---|
| `:base` | keep agent | Parent `.agent.edn` file. Child scalar options override parent scalar options; `:namespaces` merge by key. |
...
| `:doc` | rename to `:agent-description` | Short description used by generated docs and `llms/` descriptions. |
| `:system` | rename to `:system-prompt` | System prompt, either an inline string or `{:file "relative/path.txt"}`. |
| `:model` | move to LM profile as `:default-model` | Model choice is part of the LLM call policy. If a sub-agent needs a different model, give it a different `:default-lm-profile`. |
| `:provider` | rename to `:default-lm-profile` | Default LM profile for this agent. The name makes clear that `spell.api/run :lm-profile` overrides it. |
| `:budget` | rename to `:default-budget` | Default dollar budget for runs using this agent. `spell.api/run :budget` overrides it. |
...
| `:retries` | move to LM profile | API retry schedule is provider-call behavior. |
...
Inline mini specs can use agent options such as `:agent-description`, `:system-prompt`, `:default-lm-profile`, `:format`, and `:namespaces`.

Sub-agent resolution happens when the parent agent is compiled. Each `:available-agents` entry is resolved to an agent spec and compiled into a runnable function exposed in the `llms/` namespace. If the sub-agent spec has its own `:default-lm-profile`, that profile is used. Otherwise the sub-agent inherits the parent agent's resolved LM profile. Model differences should usually be represented by choosing a different `:default-lm-profile`; otherwise the sub-agent uses the inherited profile's `:default-model`.
...
|---|---|---|
| LM profile selected by an agent | agent `:default-lm-profile` | `run :lm-profile` |
| Agent selected by LM profile helpers | LM profile `:default-agent` | `run :agent` |
| Model | LM profile `:default-model` | `run :model` |
| Reasoning effort | LM profile `:default-reasoning-effort` | `run :reasoning-effort` |
| Budget | agent `:default-budget` | `run :budget` |
...
The adapter can still call a Clojure subprocess internally. Its public vocabulary is `run`, `prompt`, `init`, `lm_profile`, `agent`, `model`, `reasoning_effort`, `budget`, `depth`, `trace_dir`, `usage_tracker`, `user_reader`, and `log_writer`, not benchmark-specific request and response names.
