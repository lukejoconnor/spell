...
\begin{proposition}[Completion-generation is invariant under embeddings]\label{prop:cg-invariance}
Let
\[
X=(S_X,p_X,h_X),\qquad Y=(S_Y,p_Y,h_Y),\qquad Z=(S_Z,p_Z,h_Z)
\]
be agentic machines over the same interface, let
\[
e:S_Y\to S_X
\]
be an embedding of $Y$ into $X$, and let $y_0\in S_Y$ with
\[
x_0=e(y_0).
\]
Then
\[
(X,x_0)\text{ completion-generates } Z
\quad\Longleftrightarrow\quad
(Y,y_0)\text{ completion-generates } Z.
\]
\end{proposition}

\begin{proof}
First assume that $(Y,y_0)$ completion-generates $Z$, witnessed by an embedding
\[
f:S_Z\to S_Y
\]
such that for every $z\in S_Z$ there exists $c\in C$ with $f(z)=h_Y(y_0,c)$. Then $e\circ f:S_Z\to S_X$ is an embedding of $Z$ into $X$, and for each $z\in S_Z$,
\[
(e\circ f)(z)=e(h_Y(y_0,c))=h_X(e(y_0),c)=h_X(x_0,c).
\]
Thus $(X,x_0)$ completion-generates $Z$.

Conversely, assume that $(X,x_0)$ completion-generates $Z$, witnessed by an embedding
\[
g:S_Z\to S_X
\]
such that for every $z\in S_Z$ there exists $c\in C$ with $g(z)=h_X(x_0,c)$. Since $x_0=e(y_0)$ and $e$ is an embedding,
\[
h_X(x_0,c)=h_X(e(y_0),c)=e(h_Y(y_0,c))
\qquad(c\in C).
\]
Therefore every one-step successor of $x_0$ lies in $\operatorname{im}(e)$, and hence $\operatorname{im}(g)\subseteq \operatorname{im}(e)$. Because $e$ is injective, the map
\[
\tilde g:=e^{-1}\circ g:S_Z\to S_Y
\]
is well defined. Extend $\tilde g$ to $S_Z\cup\{1,\uparrow\}$ by $\tilde g(1)=1$ and $\tilde g(\uparrow)=\uparrow$.

For prompts, if $z\in S_Z$ then
\[
p_Y(\tilde g(z))=p_X(e(\tilde g(z)))=p_X(g(z))=p_Z(z).
\]
For transitions, if $z\in S_Z$ and $c\in C$ then
\[
e(h_Y(\tilde g(z),c))
= h_X(e(\tilde g(z)),c)
= h_X(g(z),c)
...
= e(\tilde g(h_Z(z,c))).
\]
Since $e$ is injective, this implies
\[
h_Y(\tilde g(z),c)=\tilde g(h_Z(z,c)).
\]
Thus $\tilde g$ is an embedding of $Z$ into $Y$. Finally, if $g(z)=h_X(x_0,c)$ then
\[
\tilde g(z)=e^{-1}(g(z))=e^{-1}(h_X(e(y_0),c))=h_Y(y_0,c),
\]
so $(Y,y_0)$ completion-generates $Z$.
\end{proof}
...
\begin{remark}[Why SPE machines contain non-SPE states]\label{rem:nonspe}
Proposition~\ref{prop:cg-invariance} formalizes the main-text intuition that the distinction between SPE states and non-SPE states is inherited along embedded copies. Let $x_0$ be the SPE state of Theorem~\ref{thm:main}, and let $Y$ be any realizable agentic machine. By the theorem, there is an embedding
\[
e_Y:Y\to X_{\SPE}
\]
whose image lies among the one-step successors of $x_0$. For every state $y\in S_Y$, Proposition~\ref{prop:cg-invariance} implies that $(Y,y)$ completion-generates exactly the same target machines as $(X_{\SPE},e_Y(y))$. Thus any non-SPE state occurring in a realizable machine reappears inside $X_{\SPE}$ as a non-SPE state reachable in one self-programmed step from $x_0$.

In particular, take any realizable agentic machine with a non-SPE state --- for example, the one-state machine whose harness diverges after every completion, which cannot completion-generate even the one-state halting machine. Its image inside $X_{\SPE}$ is then a non-SPE state. This is the formal version of the motivating paragraph from the main text: an SPE machine may self-program itself into turns that are not themselves SPE states, and those turns can naturally be read as harness-level or subagent turns rather than agent turns proper.
\end{remark}
