...
where `llm` calls an LLM to complete a program, and `eval` executes it. The language of this program should be flexible enough to reproduce the typical logic of an agentic harness ergonomically. 
...
One major category includes methods that allow the LM to manage its own context window. This category includes FoldGRPO, in which the agent folds or summarizes a suffix of its previous context history; AgentFold, in which the agent additionally may fold or summarize older context items anywhere in its context window; and SCULPTOR, in which the agent may fragment its context before folding or summarizing specific items. In each case, although the model controls its own context, the harness retains control over the agent loop itself.
