...
Self-programmed execution is best presented as a standard CEK evaluator together with a thin oracle wrapper. The CEK evaluator supplies the ordinary deterministic machinery: evaluator states, deterministic stepping, first-class values, and the built-in operations $\eval$ and $\quoteop$. The wrapper adds only what is specific to model interaction: a distinguished primitive $\llm$ and a boundary discipline that pauses when evaluation is poised to apply $\llm$ to a prompt value. Because quoted source is itself admitted as a completion value, seed loading can be expressed inside the source language by applying $\eval$ to an ordinary returned completion; no separate wrapper-level loading mode is needed. The prompt map $p$ and harness map $h$ are therefore read off from the pause/resume behavior of the wrapped evaluator. Because a resumed CEK computation may diverge before reaching the next boundary, the wrapper summarizes resumption by three outcomes: the next boundary, ordinary halt, or divergence.

\begin{definition}[CEK evaluator]\label{def:cek}
Fix a call-by-value language $\mathcal{L}$ with ordinary first-class data, branching, and general recursion. A \emph{CEK evaluator} for $\mathcal{L}$ is the standard deterministic abstract machine of Felleisen--Friedman and its descendants \cite{FelleisenFriedman86,AgerEtAl03}: its states are triples
\[
(e,\rho,\kappa)
\]
...
\]
Assume further that the first-class value domain of $\mathcal{L}$ is a set $V$ containing a subset $Q\subseteq V$ of quoted closed expressions, together with a total quotation operation
\[
\quoteop:V\to Q
\]
and a semantic evaluation map
\[
\eval:Q\to V
\]
satisfying
...
\qquad (v\in V).
\]
The source language also contains a built-in form, written \texttt{eval}, whose deterministic CEK behavior on quoted inputs $q\in Q$ is to return the value $\eval(q)$. We do not constrain the behavior of \texttt{eval} on non-quoted inputs beyond the ambient deterministic dynamics of the evaluator (for example, it may trap and thus reach the terminal point $1$). We work on the well-formed fragment on which every finite maximal CEK run ends at the distinguished terminal point $1$; in particular, all finite terminal outcomes, including trapped errors if present, are folded into $1$.
\end{definition}
...
\begin{enumerate}[label=(\roman*)]
    ...
    B\subseteq \Sigma
    \]
    of \emph{boundary states}, consisting of those CEK states at which deterministic evaluation is suspended because the next external action is to apply $\llm$ to an already-evaluated prompt value in $P$ and return one completion value in $C$ to the waiting continuation.
\end{enumerate}
...
\begin{itemize}
    ...
    \item a first-class outcome form $\mathsf{StepOut}$ with constructors
    \[
    \mathsf{next}:V\to \mathsf{StepOut},
    \qquad
    \mathsf{halt}\in \mathsf{StepOut},
    \]
    together with ordinary deterministic case analysis on $\mathsf{StepOut}$,
    \item a deterministic program $\mathsf{Step}$ such that evaluating $\mathsf{Step}$ on inputs $(\enc(s'),c)$ behaves as follows: it yields $\mathsf{next}(\enc(t'))$ when $h'(s',c)=t'\in S'$, it yields $\mathsf{halt}$ when $h'(s',c)=1$, and it diverges without further oracle calls when $h'(s',c)=\uparrow$.
\end{itemize}
...
\begin{proof}
If $S'=\varnothing$, the claim is vacuous. Assume henceforth that $S'\neq\varnothing$, and fix a pure CEK realization $\enc$, $\mathsf{Prompt}$, and $\mathsf{Step}$.

Choose any fixed terminal value $v_{\mathrm{done}}\in V$, and define a recursive source-level wrapper
\[
\mathsf{Loop}(z)
\]
that behaves as follows: it evaluates $\mathsf{Prompt}(z)$ to obtain a prompt value, invokes $\llm$ on that prompt, evaluates $\mathsf{Step}(z,c)$ on the returned completion $c$, and then performs case analysis on the resulting $\mathsf{StepOut}$ value, recurring on $z'$ in the case $\mathsf{next}(z')$ and returning $v_{\mathrm{done}}$ in the case $\mathsf{halt}$. This term is well formed because the underlying language has ordinary recursion and case analysis.

For each $s'\in S'$, let $e(s')\in B$ be the first boundary state reached by deterministic evaluation of $\mathsf{Loop}(\enc(s'))$. Because $\mathsf{Prompt}$ is pure and terminates with value $p'(s')$, this boundary exists and satisfies
\[
p(e(s'))=p'(s').
\]
Now resume $e(s')$ with a completion $c\in C$. By construction, the resumed CEK run continues exactly as evaluation of $\mathsf{Loop}(\enc(s'))$ after the call to $\llm$ has returned $c$. If $h'(s',c)=\uparrow$, then $\mathsf{Step}(\enc(s'),c)$ diverges, so the resumed CEK run diverges before the next boundary and hence
\[
h(e(s'),c)=\uparrow.
\]
If $h'(s',c)=1$, then $\mathsf{Step}(\enc(s'),c)$ yields $\mathsf{halt}$, so the wrapper returns $v_{\mathrm{done}}$ and the resumed run reaches the terminal point $1$. Finally, if $h'(s',c)=t'\in S'$, then $\mathsf{Step}(\enc(s'),c)$ yields $\mathsf{next}(\enc(t'))$, and the wrapper continues as $\mathsf{Loop}(\enc(t'))$ until the next boundary state, which is exactly $e(t')$. Therefore
\[
h(e(s'),c)=e(h'(s',c))
\]
for all $s'\in S'$ and $c\in C$, with the conventions $e(1)=1$ and $e(\uparrow)=\uparrow$. Since the paused CEK state $e(s')$ stores $\enc(s')$ inside this fixed wrapper template, $e$ is injective because $\enc$ is injective. Thus $X_{\SPE}$ uniformly embeds $X'$.

Choose any $s_0'\in S'$ and set
\[
q_0:=p'(s_0')\in P.
\]
Let $\mathsf{Seed}$ be the source term that calls $\llm$ on $q_0$, binds the returned completion to $y$, and then continues as $\mathsf{Loop}(\texttt{eval }y)$. Let $x_0\in B$ be the first boundary state reached by deterministic evaluation of $\mathsf{Seed}$. For each $s'\in S'$, set
\[
c_{s'}:=\quoteop(\enc(s'))\in Q\subseteq C.
\]
Then
\[
\eval(c_{s'})=\eval(\quoteop(\enc(s')))=\enc(s').
\]
So, when $x_0$ is resumed with completion $c_{s'}$, the resumed CEK run continues as evaluation of $\mathsf{Loop}(\texttt{eval }c_{s'})$, hence as evaluation of $\mathsf{Loop}(\enc(s'))$. The first boundary reached by that run is exactly $e(s')$. Therefore
\[
h(x_0,c_{s'})=e(s')
\]
for every $s'\in S'$. Hence $X'$ is completion-generated by the seed state $x_0$ of $X_{\SPE}$, and $X_{\SPE}$ completion-generates $X'$.
\end{proof}

\begin{remark}[Interpretation]
The proof reuses a classical pattern --- an injective state encoding together with a simulation/refinement-style commuting square \cite{AbadiLamport91,LynchVaandrager95,SegalaLynch95} --- and adds one SPE-specific ingredient: the seed state $x_0$ and the one-hop loading clause $h(x_0,c_{s'})=e(s')$. The point is not to install an arbitrary CEK state from a description. Rather, all embedded states $e(s')$ arise from one fixed source-level wrapper $\mathsf{Loop}$, and the seed state $x_0$ uses the ordinary completion channel together with \texttt{eval} to recover the argument $\enc(s')$ for that fixed wrapper. Because quoted source values already lie in the completion space, no special wrapper-level loading mechanism is required.
\end{remark}

\begin{remark}[How this maps to Spell]
Spell is an instance in which $Q$ is the space of quoted source forms accepted by \texttt{eval}. Under the trailing-expression discipline used by the standard wrapper, each completion appends a new trailing expression, so a seed term of the form \texttt{let y = llm ... in eval y} loads quoted source through the ordinary completion channel. The usual extend-extend-extend loop therefore produces a sequence of seed states. Earlier quoted effectful forms are shifted out of final position and become inert data, while only the new trailing form is activated by the next explicit \texttt{eval}. This is one motivation for Spell's gating discipline, under which globally effectful operations such as model calls are activated only by double evaluation.
\end{remark}
