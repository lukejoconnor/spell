...
(deftest simple-macro-expansion-test
  ...
  (testing "line-offset wraps vector literals with metadata and rejects non-vectors"
    (let [expanded (expand1 '(line-offset 7 ["a" "b"]))]
      (is (= 'quote (first expanded)))
      (is (= ["a" "b"] (second expanded)))
      (is (= {:spell/line-offset 7} (meta (second expanded)))))
    (is (ex-info-with-message?
          #"line-offset expects a vector literal"
          #(expand1 '(line-offset 7 "ab")))))
