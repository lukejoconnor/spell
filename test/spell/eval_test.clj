...
(deftest collection-access-test
  ...
  (testing "subvec preserves first-line metadata (2-arg)"
    (is (= {:spell/first-line 6}
           (meta (run-spell '(subvec (first-line 4 ["a" "b" "c" "d" "e"]) 2))))))
  (testing "subvec preserves first-line metadata (3-arg)"
    (is (= {:spell/first-line 5}
           (meta (run-spell '(subvec (first-line 4 ["a" "b" "c" "d" "e"]) 1 3))))))
  (testing "subvec without first-line unchanged"
...
(deftest value-store-test
  ...
  (testing "serialize-for-continuation with first-line vector produces a first-line wrapper"
    (let [lines (with-meta ["line one" "line two" "line three"] {:spell/first-line 10})
      ...
      (is (.startsWith ^String result "(first-line 10 ["))
      ...
      (let [parsed (first (spell.parse/read-all result))
        ...
        (is (= 10 (:spell/first-line (meta (:ok evaled))))))))

  (testing "serialize-for-continuation with first-line 1"
    (let [lines (with-meta ["alpha" "beta"] {:spell/first-line 1})
          result (eval/serialize-for-continuation lines)]
      (is (.startsWith ^String result "(first-line 1 ["))
      (is (.contains ^String result "; 1"))
      (is (.contains ^String result "; 2"))))

  (testing "serialize-for-continuation with empty first-line vector"
    (let [lines (with-meta [] {:spell/first-line 5})
          result (eval/serialize-for-continuation lines)]
      (is (= "(first-line 5 [])" result))
      (let [parsed (first (spell.parse/read-all result))
            evaled (spell-eval parsed {})]
        (is (eval/ok? evaled))
        (is (= [] (:ok evaled)))
        (is (= 5 (:spell/first-line (meta (:ok evaled)))))))))

(deftest first-line-round-trip-test
  (testing "first-line macro reconstructs vector metadata"
    (let [result (spell-eval '(first-line 5 ["a" "b"]) {})]
      (is (eval/ok? result))
      (is (= ["a" "b"] (:ok result)))
      (is (= 5 (:spell/first-line (meta (:ok result)))))))

  (testing "quote-value survives pr-str/read-string round-trip for first-line vectors"
    (let [value (with-meta ["alpha" "beta"] {:spell/first-line 17})
          quoted (eval/quote-value value)
          reparsed (first (spell.parse/read-all (pr-str quoted)))
          result (spell-eval reparsed {})]
      (is (= '(first-line 17 ["alpha" "beta"]) quoted))
      (is (eval/ok? result))
      (is (= ["alpha" "beta"] (:ok result)))
      (is (= 17 (:spell/first-line (meta (:ok result)))))))

  (testing "edit-reopen preserves first-line metadata across read/eval cycle"
    (let [lines (with-meta ["first" "second"] {:spell/first-line 40})
      ...
      (is (= 40 (:spell/first-line (meta (get (:env result) 'snippet)))))))
  ...
  (testing "subsequent serialize/parse cycles re-render numbered source"
    (let [lines (with-meta ["first" "second"] {:spell/first-line 40})
      ...
      (is (.contains ^String prefix-2 "(first-line 40 ["))
      (is (.contains ^String prefix-2 "; 40"))
      (is (.contains ^String prefix-2 "; 41"))))

  (testing "persisted first-line values re-render numbered source"
    (let [lines (with-meta ["first" "second"] {:spell/first-line 40})
          quine-form '(quine completion
                        (eval (do
                                (persist snippet lines)
...
(deftest peek-macro-and-prune-test
  ...
  (testing "peeked full binding is pruned on extension while persisted slice remains"
    (let [quine-form '(quine completion (eval (do
                          ...
                          (def fn-defn ["L2" "L3"])
                          (quote (!extend completion)))))
          result (run-spell (list 'edit-reopen (list 'quote quine-form)))
          prefix (eval/serialize-quine-prefix result)]
      (is (seq? result))
...
(deftest edit-reopen-test
  (testing "edit-reopen returns a pruned quine form"
    (let [quine-form '(quine completion (eval (do
                         (think "A" (def x 1))
                         (rethink "B" (def x 2))
                         (quote (!extend completion)))))
  ...
  (testing "edit-reopen with no rethinks passes through"
  ...
  (testing "edit-reopen with multi-arg quine preserves inert args"
  ...
  (testing "edit-reopen can prune an inert quine arg via a top-level rethink sibling"
      (let [quine-form '(quine completion
  ...
  (testing "edit-reopen with standard (2-arg) quine unchanged behavior"
  ...
  (testing "edit-reopen keeps persist with baked value after rethink pruning"
    (let [quine-form '(quine completion
          ...
          result (run-spell (list 'do
                                  ...
                                  (list 'edit-reopen (list 'quote quine-form))))]
  ...
  (testing "edit-reopen leaves non-persist forms untouched"
    (let [quine-form '(quine completion
                        (eval (do (def y (+ x 1))
                                  (quote (!extend completion)))))
          result (run-spell (list 'do
  ...
  (testing "edit-reopen leaves unbound persist forms untouched"
    (let [quine-form '(quine completion
      ...
      (is (= '(quine completion
                (eval (do (persist y (+ x 1))
                          (quote (!extend completion)))))
             (run-spell (list 'edit-reopen (list 'quote quine-form)))))))

  (testing "edit-reopen does not materialize persist inside nested fn/fn*"
    (let [quine-form '(quine completion
...
(deftest extend-macro-expansion-test
  (testing "extend expands to !llm-self with edit-reopen"
    (let [expanded (macros/spell-macroexpand-1 '(!extend completion))]
      (is (= '!llm-self (first expanded)))
      (is (= '(edit-reopen completion) (second expanded))))))
...
(deftest compact-macro-expansion-test
  (testing "compact expands to !llm-self with serialize-prefix + compact instructions"
    (let [expanded (macros/spell-macroexpand-1 '(!compact completion))]
      ...
      (is (= '(serialize-prefix (edit-reopen completion))
