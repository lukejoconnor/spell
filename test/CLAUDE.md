...
## Scenario Tests (`test/spell/scenario_test.clj`)

End-to-end tests that exercise `api/run` with `.agent.edn` files in `test/agents/`. Each agent file embeds an inline TestProvider config:

```edn
{:name one-turn
 :llms []
 :provider {:type :test
            :responses {"(quine completion (eval (do (quote (extend)) "
                        "42 "}}}
```

Simple scenarios use `:responses` (exact match). Multi-agent scenarios use `:response-rules` (substring matching) because gensym'd message names like `msg-42` make exact prompts non-deterministic.
