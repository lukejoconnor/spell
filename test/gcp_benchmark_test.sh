...
test_filter_instances_json_by_run_group() {
  ...
  local json
  json='[
    {"name":"vm-a","labels":{"run-group":"group-a"}},
    {"name":"vm-b","labels":{"run-group":"group-b"}},
    {"name":"vm-c","labels":{"run-group":"group-a"}}
  ]'

  local filtered
  filtered="$(printf '%s' "$json" | filter_instances_json_by_run_group "group-a")"
