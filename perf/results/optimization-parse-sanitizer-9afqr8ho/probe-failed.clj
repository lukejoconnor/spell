...
(defn measure [f input]
  (dotimes [_ 5] (vreset! sink (f input)))
  (let [iterations 10
    ...
    (let [elapsed (- (System/nanoTime) t0)
      ...
      (vreset! sink nil)
      {:iterations iterations :warmup-iterations 5
       :nanos-per-call (/ (double elapsed) iterations)
       :thread-allocated-bytes-per-call
       (when (and before after) (/ (double (- after before)) iterations))})))
...
(check-semantics!)
(let [results (mapv run-fixture (for [chunks [64 256] kind [:unchanged :recovery]]
                                (fixture kind chunks)))]
  (prn {:label (or (first *command-line-args*) "unlabeled")
        ...
        :interpretation "Attribution only, no candidate. Timings include call/volatile overhead and are not additive. Caller-thread allocation only; unsupported allocation counter yields nil. Inputs constructed and semantic assertions run outside measured regions. read-first sanitizes the whole input but reads only its first form."}))
