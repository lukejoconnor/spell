...
**Spell output-token breakdown in the unified JSONL is currently UNRELIABLE (TODO #240).** The `visible_output_tokens` field is `null` for Spell records and `reasoning_output_tokens` actually stores the visible count; the actual reasoning count is lost on the way into the JSONL. The `cost_usd` is still correct because it's computed in Clojure before the field mapping. If you need the real visible/reasoning split for Spell records, look at the trace-checker subagent's per-trace findings (which read the Spell runtime's `result.json` directly, not the unified JSONL) instead of aggregating from the JSONL. Codex records in the unified JSONL are fine — the bug only affects Spell records.
