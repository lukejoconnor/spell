...
3. Identify what the agent did: what approach did it take? What tools/features did it use?
4. If the task failed or errored: find the EXACT error. Quote the error message and the file + line where you found it.
5. If the task succeeded: note any interesting features used.
...
- **Spell right, baseline wrong (P3):** Look especially for evidence of interesting Spell features — orchestration (spawn/ask), context pruning (!compact, rethink), loop/recur, futures/pmap, inline math, control flow composition, or toolcall composition. These are the most compelling examples of Spell's value.
