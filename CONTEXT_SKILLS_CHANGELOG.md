...
- `src/spell/context.clj`: string-only bounded descriptor, actual stored ID, UTF-16 length, surrogate-safe preview/page boundary, executable existing-API retrieval hint, and deterministic fallback under the same aggregate cap. The final value remains the original stored object. Non-string/lazy/infinite values do not gain descriptor traversal. Tight caps may degrade to a bare reference; intrinsically oversized fixed syntax or names can still fail capacity validation.
- `src/spell/skills.clj`: removed destructive 64 KiB body truncation. Instructions come first and remain complete, including packaged resources; winning-candidate provenance moves to a footer. Existing filesystem/package discovery and precedence remain intact. This deliberately favors practical instructions over a provenance-first header. The footer identifies the winning source, not every shadowed candidate. A further provenance-tail sentence in three prompts was considered nonblocking and deferred after freeze; this report supplies the existing tail recipe below.
- `src/spell/cli.clj`: public `--context-max-chars` validates and forwards the existing run option. It is separate from output `--max-tokens`; default and minimum are unchanged.
- `src/spell/runtime.clj`, `src/spell/llm.clj`, `src/spell/user.clj`: factual receipt-site labels, not inferred execution histories. Adjacent labels refer to the following message binding. `startup: tail not run` and `pre-eval: tail not run` refer only to the current supplied/proposed tail; `wait resumed` and `dormant resumed` identify actual resume sites; `receive: not evaluated` describes explicit transformation without evaluation. None certifies earlier sends, replies or effects. Label, binding and fixed continuation share the same cap.
- Seven static skill/prompt files improve existing lossless retrieval recipes, aggregate budgeting, evidence retention, correct result-map access and canonical navigation. All ten actual skill bodies were audited; the complete disposition record is integrated below.
...
`CONTEXT_SKILLS_MODEL_FACING_INVENTORY.md` contains source-derived exact baseline/final texts and unified diffs, a file/symbol/rationale surface index, all ten audited skill bodies, all three complete prompts, generated instruction/help/descriptor/receipt implementation sources and the complete resulting API/pilot documents. It is intentionally an editable source packet, not a prose replacement. Baseline texts are read from git object `7ee774b`, final texts from the frozen checkout. Private attachments, traces and profiles are not copied or force-added.
